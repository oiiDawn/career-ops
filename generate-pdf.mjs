...
const SECTION_ALIASES = new Map([
  ...
  // Chinese — the same failure the Polish block above fixes, for the two
  // Chinese markets this repo ships modes for: Traditional (modes/zh-TW) and
  // Simplified (modes/zh), rendered through templates/cv-template.zh-minimal.html.
  // Both scripts are listed against every key because a CV written in either
  // renders through this one alias table. The vocabulary is the repo's own: the
  // `sections` payload in tests/zh-minimal-template.test.mjs and the modes'
  // wording, plus the everyday synonyms of each — titles have no single canonical
  // spelling because they come from a user-supplied `sections` override.
  ['專業摘要', 'summary'],
  ['专业摘要', 'summary'],
  ['摘要', 'summary'],
  ['個人簡介', 'summary'],
