...
### Pilot review and decision gate
...
- 试点决策读取 profile 的 `attractiveness.acceptable_line`，这是吸引力可接受线，不是旧匹配分阈值。任意硬门槛Fail→discard；否则上限低于线→deprioritize；否则门槛有Unknown、ready=false或下限低于线→verify；其余→apply。等于线视为达到。完整JD缺失不产生评分，不进入该评分队列。
- `node scoring-decisions.mjs <queue.json>` 只读生成行动列表，不写正式pipeline、不提交申请。输入 profile 路径及 opportunities（id/report/gates/ready/deadline/effort_days）；gate/readiness 必须与报告复核一致。deadline为已知ISO日期或null，effort_days为有依据的补证/准备预计工作日或null；无证据不填数。
...
- 补充定档锚点用于试点校准：compensation的1/2分别是明显/轻微低于个人底线（均不能补偿硬门槛），3达到底线但未达目标，4达到明确目标，5有显著超出目标上沿的保证收入证据；无法区分相邻档时记录分歧，不能随意加小数。
...
### 外部证据研究 — research-required-v1

适用于后续所有岗位评估；吸引力模型仍限试点。本节覆盖 oferta 中“JD 未披露工资就压缩 D 并跳过研究/提问”的规则。沿用每岗位最多 5 次搜索、主评估者单轮研究，不另建搜索服务。
