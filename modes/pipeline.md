...
2. **For each surviving pending URL**:
   ...
   e. **Execute Stage 1 only**: Follow `modes/_custom.md` → Scoring Rules: external research, attractiveness range, frozen sources and independent semantic review. Run `node scoring-report.mjs <report>` before publication. Do not generate CV/PDF/application/tracker artifacts.
   f. **Move from Pending to Scored**: `- [~] #NNN | URL | Company | Role | 吸引力 L–U/5（覆盖率P%） | Report: path`. This is a selectable pool, not a waiting gate.
