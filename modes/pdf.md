...
When a CV is reused or lightly tailored for an existing application, initialize a bundle with `npm run application:init -- --report {report-number} --company "{company}" --role "{role}" --version 1`. Keep the current JD at `jd/current.md`, the comparison JD at `jd/previous.md`, the source CV at `cv/source/original.html`, the tailored CV at `cv/tailored/v001/cv.html`, the PDF at `cv/tailored/v001/cv.pdf`, the change notes at `cv/tailored/v001/changes.md`, the reuse decision at `decision/reuse.json`, the targeted plan at `preparation/plan.json`, and drafter-reviewer records under `review/`. Resolve the application/report first with `node find.mjs {report-or-tracker-number}` so the bundle uses the report number, not an ambiguous tracker row.

Never mutate `cv.md` or the configured Reactive Resume mother resume. Reactive Resume remains the backend: it duplicates and patches the application-specific managed copy only. For pipeline roles, generate this bundle only after the user selects a specific Scored role and manually invokes Stage 2.
...
17. Build the render payload (see the **JSON Input Schema** below) from the tailored content — emit compact structured JSON, **not** full HTML markup. When `cv.output_format` is `reactive-resume`, a report/application bundle is required: write the payload to its stable `cv/tailored/vNNN/cv.json`. Otherwise write it to `/tmp/cv-{candidate}-{company}.json` as before.
18. Run `node build-cv-html.mjs {json-path} {html-path} {template}`, where `{html-path}` is the active bundle's `cv/tailored/vNNN/cv.html` or `output/cv-{candidate}-{company}.html` for a one-off CV, and `{template}` is the path printed by **Selecting the template** below (omit it to use the base template). The script owns every tag, CSS class, and HTML escaping. The HTML remains the input to the existing fact gate even when Reactive Resume renders the final PDF.
...
21. Render with the configured backend. `{report number}` is the NNN from the report filename/link (e.g. `008` for `reports/008-acme-….md`), not the tracker `#` column.
    - **Reactive Resume:** run `node reactive-resume.mjs {json-path} {pdf-path} --report={report number} --company="{company}" --role="{role}" --version={version}`. This duplicates the configured visual mother resume once per report, deterministically patches the same managed copy on reruns, downloads its PDF, stores the remote ID at `cv/reactive-resume.json`, and updates `data/pdf-index.tsv`. It fails hard when the local service, authentication, patch, or export fails; never fall back to another renderer. The configured mother resume owns the portrait, template, layout, fonts, and colors; the payload owns all factual content.
    - **HTML:** execute `node generate-pdf.mjs {html-path} {pdf-path} --format={letter|a4} --report={report number}` as before. `{pdf-path}` is the active bundle's `cv/tailored/vNNN/cv.pdf` or `output/cv-{candidate}-{company}-{YYYY-MM-DD}.pdf` for a one-off CV. Pass the report whenever the application has (or will have) one; omit it only for one-off CVs with no tracker entry. This renderer has a two-page warning threshold by default; `--max-pages=N` accepts a positive integer, and `--strict-pages` makes overflow a hard failure.
22. Report: PDF path, Reactive Resume editor link when applicable, number of pages when available, keyword coverage %, and any skill gaps from Step 4 still unaddressed
