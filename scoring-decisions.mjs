...
const ORDER = ['apply', 'verify', 'deprioritize', 'discard'];
...
export function readShortlist(pipeline, { root = ROOT, profile } = {}) {
  ...
  for (const line of section.split('\n').filter(line => /^- \[~\]/.test(line))) {
    ...
    try {
      ...
      classifyOpportunity(score, review.gates, review.ready, profile.attractiveness.acceptable_line);
  ...
  return { decisions: orderOpportunities(decisions, profile.attractiveness.acceptable_line), needs_review, invalid };
}

/** Hard failures cannot be compensated by attractiveness; unknown readiness requires verification. */
export function classifyOpportunity(score, gates, ready, threshold) {
  if (!Number.isFinite(threshold) || threshold < 1 || threshold > 5) throw new Error('explicit threshold from 1 to 5 required');
  ...
  if (!gates || Object.keys(gates).sort().join(',') !== [...GATES].sort().join(',')
    || Object.values(gates).some(s => !['Pass', 'Fail', 'Unknown'].includes(s)) || typeof ready !== 'boolean') throw new Error('complete gate states and readiness required');
  if (Object.values(gates).includes('Fail')) return 'discard';
  if (score.upper < threshold) return 'deprioritize';
  if (Object.values(gates).includes('Unknown') || !ready || score.lower < threshold) return 'verify';
  return 'apply';
}

/** Order actions by explicit deadline, effort and evidence, retaining stable IDs for ties. */
export function orderOpportunities(items, threshold) {
  const ids = new Set();
  const rows = items.map(item => {
    ...
    return { ...item, action: classifyOpportunity(item.score, item.gates, item.ready, threshold) };
  });
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.log(JSON.stringify(orderOpportunities(rows, profile.attractiveness.acceptable_line), null, 2));
  } catch (error) {
