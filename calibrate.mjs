...
export function computeCalibration(rows, journals, opts = {}) {
  ...
  // NEW attractiveness semantics: a range with low evidence coverage is too
  // wide to trust a banding point, so it is excluded (reported, not silently
  // dropped into a band). Legacy scalar rows (panel===null) are always usable.
  ...
  for (const row of rows) {
    ...
    if (row.panel && row.panel.coverage < minCoverage) { lowCoverage.push(row.num); continue; }
...
export function loadTrackerRows(appsFile) {
  ...
  for (const line of lines) {
    ...
    const attr = parseAttrScore(String(row.score ?? ''));
    rows.push({
      num: row.num,
      company: row.company || '',
      // Legacy scalar score → lower=upper=score. Attractiveness range → the
      // LOWER bound is the conservative banding point (an honest "known to be
      // at least this attractive"); midpoint is not an estimate per _custom.md.
      score: attr ? attr.lower : (() => { const f = parseFloat(String(row.score ?? '')); return Number.isFinite(f) ? f : null; })(),
      panel: attr ? { lower: attr.lower, upper: attr.upper, coverage: attr.coverage } : null,
      status: row.status || '',
    });
  }
  return rows;
...
/**
 * Parse the attractiveness-v1 score cell `吸引力 L–U/5（覆盖率P%）` into
 * {lower, upper, coverage} (coverage as 0–1 fraction). Legacy scalar cells
 * (`4.5/5`) and blanks/`N/A`/`DUP` return null so the caller falls back.
 * Recognised separators: en-dash `–`, hyphen `-`, or `~` (tolerates the
 * mojibake an em-dash becomes on a GBK round trip).
 * @param {string} cell
 * @returns {{lower:number, upper:number, coverage:number}|null}
 */
export function parseAttrScore(cell) {
  const m = /吸引力[：:\s]*([\d.]+)\s*[–\-~]\s*([\d.]+)\s*\/\s*5[^\d]*覆盖率[：:\s]*([\d.]+)\s*%/.exec(String(cell));
  if (!m) return null;
  const lower = Number(m[1]), upper = Number(m[2]), covPct = Number(m[3]);
  if (![lower, upper, covPct].every(Number.isFinite) || lower > upper) return null;
  return { lower, upper, coverage: covPct / 100 };
}
...
function renderHuman(result) {
  ...
  out.push('');
  out.push(`Resolved outcomes: ${result.resolved} · in flight (not counted): ${result.inFlight} · unscored: ${result.unscored} · low coverage: ${result.lowCoverage}`);
  out.push('');
...
function selfTest() {
  ...
  t('attractiveness range parser', parseAttrScore('吸引力 3.50–4.50/5（覆盖率75%）').lower === 3.5 && parseAttrScore('吸引力 3.50–4.50/5（覆盖率75%）').coverage === 0.75);
  const range = computeCalibration(
    [{ ...mk(10, 4.6, 'Applied'), panel: { lower: 4.6, upper: 5, coverage: 0.8 } }, { ...mk(11, 4.6, 'Applied'), panel: { lower: 4.6, upper: 5, coverage: 0.5 } }],
    new Map([[10, J('hired')], [11, J('hired')]]),
  );
  t('low-coverage attractiveness excluded', range.resolved === 1 && range.lowCoverage === 1);
